              ...
              BspCacheKeys.make.flatMap { cacheKeys =>
                Supervisor[IO](await = false).use { sup =>
                  Channel.bounded[IO, Json](64).flatMap { output =>
                    RequestLifecycle.make.flatMap { rl =>
                      ...
                      val ioStream: Stream[IO, Unit] = {
                        read
                          ...
                          .evalMap { x =>
                            ...
                            val leased = x.id.map(id => rl.run(id, handleError).map(_.flatten)).getOrElse(handleError)

                            leased.flatMap(_.map(_.asJson).traverse_(output.send))
                          }
                      }
